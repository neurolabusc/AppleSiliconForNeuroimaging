...
The M1 also includes [Apple Matrix Co-processors](https://nod.ai/comparing-apple-m1-with-amx2-m1-with-neon/) which provide [512-bit double-precision and 1024-bit single-precision](https://gist.github.com/dougallj/7a75a3be1ec69ca550e7c36dc75e0d6f) SIMD instructions. While poorly documented, these instructions can be accessed via the Accelerate framework including [vecLib](https://github.com/danielchalef/openblas-benchmark-m1), vImage, libBLAS, libBNNS, vDSP and LAPACK. The double-precision performance is particularly interesting, as Metal (which would likely be faster for [large matrices](http://machinethink.net/blog/mps-matrix-multiplication/)) currently only supports single-precision. Matrix computations form the basis of many stages of neuroimaging processing and analysis. 
