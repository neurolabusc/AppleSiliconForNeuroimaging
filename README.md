...
Matrix computations form the basis of many stages of neuroimaging processing and analysis. The M1 also includes [Apple Matrix Co-processors](https://nod.ai/comparing-apple-m1-with-amx2-m1-with-neon/) (AMX) which provide [512-bit double-precision and 1024-bit single-precision](https://gist.github.com/dougallj/7a75a3be1ec69ca550e7c36dc75e0d6f) SIMD instructions. While poorly documented, these instructions can be accessed via the Accelerate framework including [vecLib](https://github.com/danielchalef/openblas-benchmark-m1), vImage, libBLAS, libBNNS, vDSP and LAPACK. The double-precision performance is particularly interesting, as Metal currently only supports single-precision. Furthermore, the [Metal MPSMatrixMultiplication](https://developer.apple.com/documentation/metalperformanceshaders/mpsmatrixmultiplication](https://github.com/neurolabusc/Metal) function has a series of [limitations](https://github.com/neurolabusc/Metal) that make it unsuitable for many neuroimaging applications. Fortunately, [AMX outperforms Metal](https://github.com/neurolabusc/Metal) for single precision, provides double precision support, and does not carry the crippling limitations. Since Apple uses the same function names (e.g. cblas_dgemm) as other Basic Linear Algebra Subprograms (BLAS) libraries available for Windows and Unix, one can exploit this new hardware by simpling compiling software linked to the Apple Accelerate framework.  
...
The graph below shows time to complete this computation, with lower values illustrating better performance. The `32` and `64` labels show 32-bit single-precision and 64-bit double precision compuation. The `scalar` values is the [popular and intuitive C solution](https://github.com/afni/afni/blob/dab76581b2ddef36dd4d5974f122e9759732c5c5/src/matrix.c#L696) without [any](https://www.cs.cornell.edu/~bindel/class/cs5220-s10/slides/lec03.pdf) common [optimizations](http://apfel.mathematik.uni-ulm.de/~lehn/sghpc/gemm/). The `blas` value shows performance for the optimized `cblas_dgemm()` function (using [libblas](https://packages.ubuntu.com/bionic/libblas-dev) for the 3900X and [Apple's Accelerate framework](https://developer.apple.com/documentation/accelerate/1513282-cblas_dgemm?language=objc) for the M1). Ryzen performance is also provided for the free but not open-source Intel [Math Kernel Library](https://en.wikipedia.org/wiki/Math_Kernel_Library) (`MKL`) and for the [NVidia CUDA](https://solarianprogrammer.com/2012/05/31/matrix-multiplication-cuda-cublas-curand-thrust/) (`cuda`) library to use the GTX 1080 Ti graphics card (see the cuda_mmul.cu source code). The timings for CUDA include the substantial memory transfer times between the CPU and GPU memory. For completeness, the included source code also evaluates the [Accelerate framework's](https://developer.apple.com/documentation/accelerate/vdsp) `vDSP_mmulD()` function, but this is omitted from the graph as it is just as fast as `cblas_dgemm()`. Finally, the source code reports the maximum difference observed between the `scalar` and `blas` results, in this case a negligible 2e-14 (one does not expect identical results, as optimized methods can use fused multiply-add operations that [reduce rounding errors](https://stackoverflow.com/questions/15933100/how-to-use-fused-multiply-add-fma-instructions-with-sse-avx)).
