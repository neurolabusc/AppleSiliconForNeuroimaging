...
The graph below shows time to complete this computation, with lower values illustrating better performance. The `scalar` values is the [popular and intuitive C solution](https://github.com/afni/afni/blob/dab76581b2ddef36dd4d5974f122e9759732c5c5/src/matrix.c#L696) without any [optimizations](http://apfel.mathematik.uni-ulm.de/~lehn/sghpc/gemm/). The `blas` value shows performance for the optimized `cblas_dgemm()` function (using [libblas](https://packages.ubuntu.com/bionic/libblas-dev) for the 3900X and [Apple's Accelerate framework](https://developer.apple.com/documentation/accelerate/1513282-cblas_dgemm?language=objc) for the M1). For completeness, the included source code also evaluates the [Accelerate framework's](https://developer.apple.com/documentation/accelerate/vdsp) `vDSP_mmulD()` function, but this is omitted from the graph as it is just as fast as `cblas_dgemm()`. Fiinally, the source code reports whether the maximum difference observed between the `scalar` and `blas` results, in this case a negligible 2.13163e-14 (one does not expect identical results, as optimized methods can use fused multiply-add operations that reduce rounding errors).
